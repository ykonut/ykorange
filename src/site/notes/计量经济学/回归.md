---
{"dg-publish":true,"permalink":"/计量经济学/回归/","tags":["因果推断"],"created":"2025-09-12 19:18","updated":"2026-01-21 16:12","dg-note-properties":{"date created":"2025-09-12 19:18","date modified":"2026-01-21 16:12","tags":["因果推断"]}}
---
...
根据 [[计量经济学/Frisch-Waugh-Lovell Theorem\|Frisch-Waugh-Lovell Theorem]] 可知，$\tau_{R}$ 也相当于 $Y$ 对 $\tilde{D}$ 回归的系数。其中，$\tilde{D}$ 是 [[计量经济学/Linear Projection Model\|Linear Projection Model]] $D=X^{\mathsf{T}}\beta'+\tilde{D}$ 当中的 Projection Error。不过，根据条件期望的定义，$X$ 是离散变量就意味着 $E[D\mid X]=X^{\mathsf{T}}\beta'$，即条件期望函数必然是线性的。因此 $\tilde{D}=D-E[D\mid X]$，即 $\tilde{D}$ 还是 [[计量经济学/线性CEF模型#CEF Error\|CEF Error]]，也就满足相关性质。如果 $X$ 是连续变量，要使用 CEF Error 的相关性质就需要明确假设 $E[D\mid X]$ 关于 $X$ 是线性的了 (Aronow and Samii, 2016）。
